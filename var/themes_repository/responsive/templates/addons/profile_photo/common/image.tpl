...
{hook name="common:image"}
    ...
    {else}
        <div class="no-image {$no_image_css_class}" style="width: {$image_width|default:$image_height}px; height: {$image_height|default:$image_width}px;">{include_ext file="addons/profile_photo/common/icon.tpl" class="glyph-image" title=__("no_image")}</div>
    {/if}
